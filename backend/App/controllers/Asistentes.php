...
html;
            ...
            else
            {
                ...
                {
                    foreach (GeneralDao::getBuscarEstatusCompraEmail($value['usuario'] ) as $key => $value_busca_beca) { //IR A BUSCAR EL ESTATUS DE PAGO
                        if(empty($value_busca_beca['status']))//Se pregunta si esta vacia, significa que solo se registro y no ha pagadp
                        {
                            $permiso_impresion .= <<<html
